# Ultimate Structured Curriculum
## Python → FastAPI → Production Agentic AI (2026)

---

[MATH CS AI Compendium](https://github.com/HenryNdubuaku/maths-cs-ai-compendium)

## Phase 0 – Setup & Mindset

- Use Cursor or Claude Code from day one.
- Tip: Calculate the Cost of the Model per Token - input/Output. Use LLM Cost calculator.

---

## Phase 1 – Python Fundamentals

**Videos**
- [Learn Python – Interactive Course 2026 by FCC](https://www.youtube.com/watch?v=kLZgQWjnUz0)

---

## Phase 2 – SQL and Database

**Videos**
- [SQL - Complete Learn SQL Beginner to Advanced in Under 4 Hours](https://www.youtube.com/watch?v=OT1RErkfLNQ)
- [Intro to Postgres - by Database School](https://databaseschool.com/series/intro-to-postgres/videos/203)
- [Intro to Supabase – Full Tutorial for Beginners by FCC](https://www.youtube.com/watch?v=Q7P20fHJlm4)
- [DATABASE Design Course](https://youtu.be/h0j0QN2b57M?si=jVOkPTDRZE9adrI8)
- [Database Design & Data Modeling Full Course For Beginners](https://www.youtube.com/watch?v=_jf7KZqne-8)

**Practice & Tutorials**
- [SQL Tutorials](https://www.thoughtspot.com/sql-tutorial)
- [SqlBolt - Practice](https://sqlbolt.com/)
- [ChaiSql - Practice - Labs](https://sql.chaicode.com/)

**Reading**
- [Scaling PostgreSQL to power 800 million ChatGPT users](https://openai.com/index/scaling-postgresql/)

---

## Phase 3 – FastAPI

**Videos**
- [Python API Development - Comprehensive Course for Beginners by FCC](https://www.youtube.com/watch?v=0sOvCWFmrtA)
- [FAST API - Playlist](https://www.youtube.com/playlist?list=PLKnIA16_RmvZ41tjbKB2ZnwchfniNsMuQ)
- [FastAPI Tutorial for Beginners – Full Course](https://www.youtube.com/watch?v=VirndPTeRaw)
- [FastAPI Beyond CRUD Full Course - A FastAPI Course](https://www.youtube.com/watch?v=TO4aQ3ghFOc)
- [FASTAPI for AI Engineers](https://www.youtube.com/watch?v=IcM2w2VKdbA)

---

## Phase 4 – UI Development

### Gradio Interface
- [Gradio](https://gradio.app/guides/quickstart)
- [Create a Simple LM application](zulip.sair.foundation/#narrow/channel/9-general/topic/submissions.20partially.20accepted/with/3263)
- [Gradio Course - Create User Interfaces for Machine Learning Models by FCC](https://www.youtube.com/watch?v=RiCQzBluTxU)
- [Python Gradio Tutorial](https://www.youtube.com/playlist?list=PLMi6KgK4_mk3DH0Mze62_wyxEmg4R9dF9)

### Streamlit UI
- [Get Started](https://discuss.streamlit.io/t/streamlit-quests-getting-started-with-streamlit/33534)
- [Video Tutorial](https://www.youtube.com/watch?v=LjZTXKaPz2M&list=PLc2rvfiptPSSpZ99EnJbH5LjTJ_nOoSWW&index=15)

### Full-stack / HTMX
- [Using HTMX with FastAPI](https://testdriven.io/blog/fastapi-htmx/)
- [Full Stack APP tutorial](https://www.youtube.com/watch?v=sT3WSkMyCXA)

**Tip:** Use Lovable.dev or similar to build a Biloperate site or UI. Also Use Streamlit UI Skills.  
- [Lovable.dev tutorial](https://youtu.be/qxygwvBoBTc?si=ZpH-h2U4J4WlNEOS)

---

## Phase 5 – Claude Code, Cursor, Vibe Coding & Agentic AI Development

- [Olly Rosewell - Sessions](https://www.youtube.com/@OBRosewell/videos)
- [Build with Hussain Day1-4 Agentic AI Development Hands-on Workshops](https://www.youtube.com/playlist?list=PLI1bSTEnI2mI)
- [How Senior Engineers Actually Build with AI in 2026 | Build a Splitwise AI Clone](https://www.youtube.com/watch?v=uqX2hY7CseU)
- [JS Mastery : Agentic Development as Senior Engineers](https://www.youtube.com/playlist?list=PL6QREj8te1P6JDXgrCTf-7cuOy8w5jCCZ)
- [Claude Official Playlist - Developers](https://www.youtube.com/playlist?list=PLf2m23nhTg1PBzCb-nOGFH6NFYSkkVuZ5)
- [CC For Everyone](https://ccforeveryone.com/)
- [Claude Code Full Course – Autonomous Goals, MCP, and VS Code Setup](https://www.youtube.com/watch?v=7l6bXLAKyEI)

---

## Phase 6 – Intro to LLMs and Prompt Engineering

**Playlists & Courses**
- [Large Language Models (LLMs) - Shaw Talebi](https://www.youtube.com/playlist?list=PLz-ep5RbHosU2hnz5ejezwaYpdMutMVB0)
- [LLMs From Scratch - Book](https://sebastianraschka.com/llms-from-scratch/)
- [Create a Large Language Model from Scratch with Python - FCC Tutorial](https://www.youtube.com/watch?v=UU1WVnMk4E8)

**Videos**
- [Pre Recorded Resource - Intro to LLMs (Andrej Karpathy)](https://www.youtube.com/watch?v=zjkBMFhNj_g)
- [Pre Recorded Resource - Transformers (how LLMs work) explained visually](https://www.youtube.com/watch?v=wjZofJX0v4M&t=1s)
- [Advanced Resource: Building GPT from scratch](https://www.youtube.com/watch?v=kCc8FmEb1nY)
- [Resource - LLM Leaderboard](https://arena.ai/leaderboard)
- [Advanced Resource: LLM Foundations](https://karpathy.ai/zero-to-hero.html)

**Repos & Guides**
- [Github Repo - Large Language Models from Scratch](https://github.com/rasbt/LLMs-from-scratch)
- [AI Engineering from Scratch](https://aiengineeringfromscratch.com/)
- [Official Prompt Engineering Guide](https://www.promptingguide.ai/)
- [AI Engineering 101 - Saurav Prateek](https://github.com/SauravP97/AI-Engineering-101)
- [Dair AI - Prompt-Engineering-Guide -- repo](https://github.com/dair-ai/Prompt-Engineering-Guide)

**Tip:** Calculate the Cost of the Model per Token - input/Output. Use LLM Cost calculator.

---

## Phase 7 – Tool Calling and MCP

- [Introduction to function calling - Google](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/tools/function-calling)
- [Tool Calling, Explained: How AI Agents Decide What to Do Next](https://towardsdatascience.com/tool-calling-explained-how-ai-agents-decide-what-to-do-next/)
- [Anthropic - advanced Tool Use](https://www.anthropic.com/engineering/advanced-tool-use) — **Programmatic Tool Calling**
- [End to End MCP Tutorial by FCC](https://www.youtube.com/watch?v=DosHnyq78xY&)
- [Ultimate MCP Tutorial | Learn Model Context Protocol and Deploy your MCP Server](https://www.youtube.com/watch?v=DAuZuj0BUZA)
- [MCP Into and Understand AO](https://www.youtube.com/watch?v=Ek8JHgZtmcI&t=48s)
- [MCP Agentic AI Crash Course With Python](https://www.youtube.com/watch?v=MDBG2MOp4Go)
- [MCP Crash Course: What Python Developers Need to Know](https://youtu.be/5xqFjh56AwM?si=7fOsrt_lc0r_ejhO)

---

## Phase 8 – RAG (Retrieval Augmented Generation) from First Principles

- [Deep Dive](https://towardsdatascience.com/retrieval-augmented-generation-rag-an-introduction/)
- [Build a Simple RAG Pipeline in 30 Minutes!](https://www.youtube.com/watch?v=gcqp3Fbv4_o)
- [Is RAG Dead in 2026? | Build Local RAG from First Principles](https://www.youtube.com/watch?v=jdknLDkBS3k)
- [Learn RAG From Scratch – Python AI Tutorial from a LangChain Engineer](https://www.youtube.com/watch?v=sVcwVQRHIc8)
- [Improve RAG With Simple API](https://youtu.be/VX1yG3X3pY4?si=TrEgz5Mq6o1rDIKi)
- [Hybrid Search](https://youtu.be/TbtBhbLh0cc?si=Q0etmmmI1sqR5qgJ)
- [Pgai Vector Embeddings](https://youtu.be/8oTnUtFYAes?si=fV3PNuSG-psaVZnM)
- [RAG by Cohere](https://docs.cohere.com/docs/retrieval-augmented-generation-rag)
- [RAG Cookbooks - github](https://github.com/athina-ai/rag-cookbooks)
- [Practical RAG Use Case](https://www.youtube.com/watch?v=jqd6_bbjhS8&t=6s)
- [From RAG Prototype to Production: Why We Wrote “Hands-On RAG for Production”](https://www.vectara.com/blog/from-rag-prototype-to-production-why-we-wrote-hands-on-rag-for-production)

---

## Phase 9 – Advanced RAG, Agentic RAG and Memory

**Videos**
- [How to Get Your Data Ready for AI Agents (Docs, PDFs, Websites)](https://www.youtube.com/watch?v=9lBTS5dM27c&t=36s)
- [Building Agentic RAG From Scratch in Pure Python](https://www.youtube.com/watch?v=RxwjoegpI98&t=1s)

**Repos & Tools**
- [production-grade RAG system using industry best practices - Jamwithai](https://github.com/jamwithai/production-agentic-rag-course)
- [Beyond Naive RAG: Advanced Techniques for Building Smarter and Reliable AI Systems](https://towardsdatascience.com/beyond-naive-rag-advanced-techniques-for-building-smarter-and-reliable-ai-systems-c4fbcf8718b8/)
- [Advanced RAG Techniques: an Illustrated Overview](https://pub.towardsai.net/advanced-rag-techniques-an-illustrated-overview-04d193d8fec6)
- [Ragas](https://www.ragas.io/)
- [PageIndex](https://pageindex.ai/)
- [neo4j](https://neo4j.com/)
- [LLM Wiki by Andrej Karpathy]()
- [DeepEval](https://github.com/confident-ai/deepeval)
- [Jamwithai](https://www.youtube.com/@jamwithai) — go through this channel

**Memory Systems**
- [Reverse Engineered Claude's Memory System](https://manthanguptaa.in/posts/claude_memory/)
- [Reverse Engineered ChatGPT's Memory System](https://manthanguptaa.in/posts/chatgpt_memory/)
- [Towards Human like Memory for AI Agents](https://manthanguptaa.in/posts/towards_human_like_memory_for_ai_agents/)
- [Practical Guide to Memory](https://towardsdatascience.com/a-practical-guide-to-memory-for-autonomous-llm-agents/)

---

## Phase 10 – Agentic AI and Multi-Agent Systems

**Videos**
- [Agentic AI CC by Codebasics](https://youtu.be/D74el9mvNak?si=RGp6rNgbH806PZ6g)
- [DaveEbbelar](https://youtu.be/T1Lowy1mnEg?si=Jn94DwJPAglLgPDk)
- [SelfLearning AI Agents](https://youtu.be/ynhl8KjjS3Y?si=6xMlP_C1zuXC7NkD)
- [Build Effective AI Agents](https://youtu.be/tx5OapbK-8A?si=umEZP1Yq9vh49JFU)
- [Build Agent with Pydantic](https://youtu.be/zcYtSckecD8?si=O6nnVRZEa0aXHSSZ)
- [AI Agent in Pure Python](https://youtu.be/bZzyPscbtI8?si=wufStmFuM8lwNwFA)
- [AI Agent Actually Work in Python](https://youtu.be/Q3Gb7Rjre3U?si=jGrOMs6myAfaVO5V)
- [Agetic AI System Design and Overview](https://youtu.be/ZIAzZtKWmbI?si=EcrvExwbRPOStGPP)
- [The Engineering System for AI Agents](https://youtu.be/Vok_nReMFaU?si=e7CZ53nxUBLGScwG)
- [Stanford : CS329A Self-Improving AI Agents](https://www.youtube.com/playlist?list=PLangBM27OtEA)

**Reading**
- [React paper](https://arxiv.org/pdf/2210.03629)
- [A practical guide to building agents - OpenAI](https://openai.com/business/guides-and-resources/a-practical-guide-to-building-ai-agents/)
- [Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)
- [Bash Computer Use Agent with NVIDIA Nemotron](https://developer.nvidia.com/blog/create-your-own-bash-computer-use-agent-with-nvidia-nemotron-in-one-hour/)

**Repo**
- [Hands-on workshop: Build a multi-agent AI system from scratch - AIE](https://github.com/iusztinpaul/designing-real-world-ai-agents-workshop)
- [Agents in Action](https://github.com/traversaal-ai/agents-in-action)
- [AI Agents From Scratch](https://github.com/pguso/ai-agents-from-scratch)
**Resources** 
- [ml-intern](https://huggingface.co/spaces/smolagents/ml-intern)
- [smolagents - huggingface](https://huggingface.co/docs/smolagents/en/index)

---

## Phase 11 – Evals and Guardrails

**Videos**
- [LLM Evals](https://youtu.be/a3SMraZWNNs?si=vOaJ6DHLTUEC0PwB)
- [AI Agents Guardrails Simplified](https://youtu.be/9Ek2cvIHq1c?si=lLb8ZTM5OKyKv-qw)
- [You're Not AI-Fluent Until You Understand Evals](https://www.youtube.com/watch?v=zLeG-XJtbIE)
- [Post-training Agents](https://www.youtube.com/playlist?list=PLo2EIpI_JMQvQZm-kVlz4wY1vWF0LBcf5)

---

## Phase 12 – Fine-tuning LLMs/SLMs + Responsible AI, RLM, Harness & Advanced

**Videos**
- [Intro to Fine-Tuning Large Language Models](https://youtu.be/H-oCV5brtU4?si=NNQzNlBrPzippgcr)
- [LLM Finetuning by FCC](https://youtu.be/iOdFUJiB0Zc?si=LPAdtjT5tasSU3tQ)
- [LLM Finetuning Hands on Lab 12 hrs -- FCC](https://youtu.be/CcrC5zSv1iA?si=nkQSMBXH5lEIk-bU)
- [FineTuning SLM Part-1](https://youtu.be/pJIfSHwVT1E?si=z7MwNDrIJYljFM8K)
- [Harness Engineering Practical Demo](https://youtu.be/vBS6z9z-7hw?si=Do1TWI5p_XKTrpCj)
- [RLM Explained](https://youtu.be/5RAFKES5J6E?si=jOM1N3iuiJRhbYju)
- [RLM From Scratch](https://youtu.be/nxaVvvrezbY?si=Rqox2Y5qt_brARRP)

**Reading & Overview**
- [RLM Paper](https://arxiv.org/abs/2512.24601)
- [A pragmatic guide to LLM evals for devs](https://newsletter.pragmaticengineer.com/p/evals)
- [LLaMa Guard -- official](https://ai.meta.com/research/publications/llama-guard-llm-based-input-output-safeguard-for-human-ai-conversations/)
- [LLaMa Guard - HF](https://huggingface.co/meta-llama/Llama-Guard-3-8B)
- [OpenAI Guardrails](https://guardrails.openai.com/)

---

## Ongoing Engineering Topics (keep open forever)

- Prompt Engineering
- Context Engineering
- Harness Engineering
- Loop Engineering
- Graph Engineering
- Inference Engineering

**Blogs to follow**
- OpenAI Engineering blog
- Anthropic Engineering blog
- Amplabs blogs
- Meta AI Engineering blogs

---

# Advanced Phases (Previously Missing)

These are the critical gaps for reaching senior / production level in 2026.

---

## Phase 13 – Observability & Production Debugging

- Agent tracing (LangSmith, Arize Phoenix, OpenTelemetry)
- Cost & latency tracking per step / trajectory
- Token usage dashboards
- Failure analysis of tool calls
- Logging structured outputs and decisions

---

## Phase 14 – Inference Optimization & Serving

- vLLM / TensorRT-LLM / SGLang
- Quantization (GGUF, AWQ, GPTQ, EXL2)
- Continuous batching & speculative decoding
- Proper streaming responses
- GPU vs CPU cost trade-offs

---

## Phase 15 – AI Infrastructure & Deployment

- Deep async patterns + background workers (ARQ, Celery, Redis queues)
- Multi-stage Docker builds for AI services
- Deployment platforms (Modal, Fly.io, Railway, basic Kubernetes)
- Autoscaling for LLM workloads
- Secrets management & environment strategy

---

## Phase 16 – Security, Cost Engineering & Advanced Context

**Security**
- Prompt injection defenses
- Tool / MCP authorization & sandboxing
- Data leakage prevention
- Rate limiting + abuse detection
- PII redaction

**Cost Engineering**
- Semantic caching
- Model cascading / routing (cheap model first)
- Batch inference
- Budget controls & alerts

**Advanced Context Engineering**
- Context compression & hierarchical memory
- Long-context vs RAG decision making
- Dynamic context assembly
- State management for long-running agents

**Multi-modal**
- Vision + tool calling
- Document + image understanding
- Basic audio / voice agents

---

## Recommended Learning Order

1. Phases 1–4 → Solid backend + UI foundation  
2. Phases 5–9 → LLM + RAG + Agents mastery  
3. Phases 10–12 → Evals, fine-tuning, harness  
4. Phases 13–16 → Production hardening  

**Capstone Ideas**
1. FastAPI + Postgres social API with auth  
2. Streamlit / Gradio UI on top  
3. Add RAG over documents + posts  
4. Turn into agent with MCP tools + memory  
5. Add observability, evals, guardrails, and deploy  

---

*Last updated: September 18, 2026*  
*All original resources preserved. Advanced production topics added.*